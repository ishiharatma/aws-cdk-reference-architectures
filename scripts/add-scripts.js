...
try {
  ...
  const newScripts = {
    ...
    "validate": "cross-env COMMIT_HASH=$(git rev-parse --short HEAD) cdk validate '**' --unstable=validate --version-reporting false --asset-metadata false -c project=${PROJECT} -c env=${ENV} --profile ${PROJECT}-${ENV}",
    ...
    "d:stage:destroy:all": "cross-env cdkd destroy '**' -c project=${PROJECT} -c env=${ENV} --profile ${PROJECT}-${ENV}",
    "lint": "eslint \"**/*.ts\"",
