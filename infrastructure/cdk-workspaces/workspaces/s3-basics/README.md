...
├── test/
│   ├── compliance
│   │      └── cdk-nag.test.ts     # Test code (explained in later exercises)
│   ├── snapshot
│   │      └── snapshot.test.ts    # Test code (explained in later exercises)
│   └── unit
│          └── s3-basics.test.ts   # Test code (explained in later exercises)
├── cdk.json
├── package.json
└── tsconfig.json
```
...
⚠️: If you deploy this code to AWS and delete it, you will need to manually delete the `CDKDefault` bucket.
...
2. Staged transition: STANDARD → STANDARD_IA → GLACIER INSTANT RETRIEVAL
