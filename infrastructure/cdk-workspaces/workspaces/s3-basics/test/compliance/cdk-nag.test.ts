...
describe("CDK Nag AwsSolutions Pack", () => {
  let app: cdk.App;
  let stack: S3BasicStack;

  beforeAll(() => {
    // Execute CDK Nag checks
    app = new cdk.App();

    stack = new S3BasicStack(app, `${projectName}-${envName}`, {
      project: projectName,
      environment: envName,
  ...
  test("S3 resources comply with AWS Solutions checks", () => {
 ...
 */
function applySuppressions(stack: S3BasicStack): void {
  ...
  NagSuppressions.addStackSuppressions(
    ...
    [
      ...
      },
      {
        id: "AwsSolutions-S10",
        reason:
          "These are example S3 buckets for demonstration and SSL is not required.",
      },
    ],
    true,
  );
