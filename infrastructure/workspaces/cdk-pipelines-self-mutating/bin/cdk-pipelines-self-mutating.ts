...
import { CdkPipelinesSelfMutatingStage } from 'lib/stages/cdk-pipelines-self-mutating-stage';
...
// Reference/learning stack: the repository is deleted with the stack outside production.
const isAutoDeleteObject = envName !== Environment.PRODUCTION;
const isTerminationProtection = envName === Environment.PRODUCTION;

const stage = new CdkPipelinesSelfMutatingStage(app, `CdkPipelinesSelfMutating${pascalCase(envName)}`, {
  project: pjName,
  environment: envName,
  env: defaultEnv,
