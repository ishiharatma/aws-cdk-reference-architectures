...
describe('ApigwLambdalithStack', () => {
  ...
  beforeAll(() => {
    ...
    const stack = new ApigwLambdalithStack(app, 'ApigwLambdalith', {
      project: projectName,
      environment: envName,
  ...
  describe('Lambda', () => {
    test('exactly one function serves the whole API (Lambdalith)', () => {
    ...
    test('function receives the table name by environment variable', () => {
      template.hasResourceProperties('AWS::Lambda::Function', {
        Environment: { Variables: { TABLE_NAME: Match.anyValue(), ENVIRONMENT: 'test' } },
      });
    });
  ...
  describe('API Gateway', () => {
    test('one REST API with a greedy proxy resource', () => {
      template.resourceCountIs('AWS::ApiGateway::RestApi', 1);
      template.hasResourceProperties('AWS::ApiGateway::Resource', { PathPart: '{proxy+}' });
    });

    test('proxy resource uses the ANY method wired to a Lambda AWS_PROXY integration', () => {
