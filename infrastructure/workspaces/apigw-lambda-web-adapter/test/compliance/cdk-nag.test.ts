...
describe('CDK Nag AwsSolutions Pack', () => {
  let stack: ApigwLambdaWebAdapterStack;

  beforeAll(() => {
    const app = new cdk.App();
    stack = new ApigwLambdaWebAdapterStack(app, `${projectName}-${envName}-apigw-lambda-web-adapter`, {
      project: projectName,
      environment: envName,
 ...
 * *Lambda integration style* (one function running a full Express.js server
 * behind the Lambda Web Adapter). Adding a Cognito/JWT/IAM authorizer, a WAFv2
 * Web ACL, and model-based request validation is documented in the README as
 * an environment-specific step.
 */
  ...
  NagSuppressions.addStackSuppressions(
    ...
    [
      ...
      },
      {
        ...
        reason:
          'Request validation is performed inside the Express.js application; API Gateway model validation would ' +
          'duplicate it for this reference.',
      },
      {
