...
function suppressAppStack(stack: AppStack): void {
    ...
    NagSuppressions.addResourceSuppressionsByPath(stack, `${p}/Distribution/Resource`, [
        ...
        },
        {
            ...
            reason: 'WAF integration is out of scope for this EC2 chaos reference pattern.',
        },
        {
    ...
    // EC2 instance role: AmazonSSMManagedInstanceCore is required for SSM agent
    // registration (needed for FIS C-2 CPU stress and C-4 network blackhole actions).
        ...
        [
            {
                id: 'AwsSolutions-IAM4',
                reason:
                    'AmazonSSMManagedInstanceCore is required for SSM agent registration on EC2 instances. ' +
                    'The SSM agent is the mechanism FIS uses to inject CPU stress (C-2) and network blackhole (C-4).',
                appliesTo: [
    ...
    // These are internal operational buckets for a chaos demo, not production data stores.
    NagSuppressions.addStackSuppressions(
        stack,
        [
            {
    ...
    // - AwsSolutions-L1: Lambda runtime may not be recognised as latest by cdk-nag (CDK-internal resource)
    // - AwsSolutions-IAM4: uses AWSLambdaBasicExecutionRole (accepted baseline for CDK custom resources)
    // - AwsSolutions-IAM5: wildcard on S3 and CloudWatch for BucketDeployment operations
    NagSuppressions.addStackSuppressions(
        stack,
        [
            {
...
function suppressFisStack(stack: FisStack): void {
    ...
    // ec2:DescribeInstances, ssm:GetCommandInvocation, etc. cannot be scoped to a resource.
        ...
        [
            {
                ...
                reason:
                    ...
                    'restrictions. ec2:DescribeInstances and SSM monitoring actions also require wildcard resources.',
            },
            {
