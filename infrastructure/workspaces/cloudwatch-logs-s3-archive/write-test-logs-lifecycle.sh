...
#######################################
# write-test-logs-lifecycle.sh
#
# Writes test log events into the CloudWatch Log Group created by
# CloudwatchLogsS3ArchiveLifecycleStack (Stack 2), to exercise the
# CloudWatch Logs -> Firehose -> S3 archive pipeline with tiered
# lifecycle rules.
#
# The log group is discovered by prefix:
#   /<project>/<env>/lifecycle-
# (CloudwatchLogsS3ArchiveLifecycleStack creates a single log group named
#  /<project>/<env>/lifecycle-<suffix>)
#
# Usage:
#   ./write-test-logs-lifecycle.sh --project PROJECT --env ENV [OPTIONS]
#
# Examples:
#   ./write-test-logs-lifecycle.sh --project myproject --env dev
#   ./write-test-logs-lifecycle.sh --project myproject --env dev --count 20 --message "load test"
#######################################
...
CloudwatchLogsS3ArchiveLifecycleStack (/<project>/<env>/lifecycle-*).
...
EXAMPLES:
    # Write 1 test event to the lifecycle-stack log group
...
# Populates the LOG_GROUPS array with log groups under /<project>/<env>/lifecycle-
find_log_groups() {
    local prefix="/${PROJECT}/${ENVIRONMENT}/lifecycle-"
    ...
    if [ "${#LOG_GROUPS[@]}" -eq 0 ] || [ -z "${LOG_GROUPS[0]}" ]; then
        ...
        print_message "$YELLOW" "Make sure CloudwatchLogsS3ArchiveLifecycleStack has been deployed for this project/env"
        exit 1
    fi
...
main() {
    print_message "$BLUE" "==================================================="
    print_message "$BLUE" "  CloudWatch Logs Test Data Writer (Lifecycle Stack)"
    print_message "$BLUE" "==================================================="
    echo
