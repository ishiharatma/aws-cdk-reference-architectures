...
function applyExportSuppressions(stack: cdk.Stack): void {
    ...
    // Python 3.14 is the latest stable Python runtime available in Lambda at the
    ...
    NagSuppressions.addResourceSuppressionsByPath(
        ...
        [
            {
                id: 'AwsSolutions-L1',
                reason: 'Python 3.14 is the latest available Python runtime for Lambda at the time of authoring; the cdk-nag package in this workspace may not yet recognise it as the latest.',
            },
        ],
    );

    // EventBridge Scheduler's LambdaInvoke target role is auto-generated by CDK and
    // granted lambda:InvokeFunction on the export function ARN and its version/alias
    // wildcard (arn:...:function:<name>:*). The ':*' suffix trips IAM5. Both the
    // Scheduler role and the referenced function logical ID carry non-deterministic
    // hashes, so this is suppressed at the stack level with a regex `appliesTo`
    // pinned to the export function's ARN wildcard only.
    NagSuppressions.addStackSuppressions(stack, [
        {
            id: 'AwsSolutions-IAM5',
            reason: 'CDK-generated EventBridge Scheduler target role invokes exactly one Lambda; the arn:...:function:<name>:* wildcard only covers that function\'s versions and aliases.',
            appliesTo: [{ regex: '/^Resource::<ExportTaskFunction[0-9A-Fa-f]{8}\\.Arn>:\\*$/g' }],
        },
    ]);
}
