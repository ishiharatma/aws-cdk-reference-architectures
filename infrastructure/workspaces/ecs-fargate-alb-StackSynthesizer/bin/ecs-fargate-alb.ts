...
import { PermissionsBoundary, ManagedPolicy } from 'aws-cdk-lib/aws-iam';
...
// CDK_USE_CALLER_ROLE=true のとき Bootstrap ロールを介さず実行者ロールで直接デプロイする
// SCP で PowerUserAccess が PermissionsBoundary として強制されている環境で deploy-role の
// PassRole が境界によりブロックされる問題を回避するための手段
// BootstraplessSynthesizer と異なりファイルアセットをサポートするため VPC 等の
// Lambda バックドカスタムリソースも利用可能
const synthesizer = process.env.CDK_USE_CALLER_ROLE === 'true'
