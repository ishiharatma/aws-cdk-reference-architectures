...
describe('CDK Nag AwsSolutions Pack', () => {
    let stack: Route53PhzDelegationStack;

    beforeAll(() => {
        const app = new cdk.App();
        stack = new Route53PhzDelegationStack(app, 'Route53PhzDelegation', {
            project: projectName,
            environment: envName,
            env: defaultEnv,
 ...
 * (flow-logs-off) VPCs, and security groups whose CIDR is an intrinsic VPC reference.
 */
function applySuppressions(stack: Route53PhzDelegationStack): void {
    ...
    // EC2 instances (BIND9 forwarder + Hub test instance).
    for (const instanceId of ['OnPremDnsForwarder', 'HubTestInstance']) {
        NagSuppressions.addResourceSuppressionsByPath(stack, `${pathPrefix}/${instanceId}/SecurityGroup/Resource`, [
            {
                id: 'CdkNagValidationFailure',
                reason: 'The security group references a VPC CIDR via an intrinsic function (Fn::GetAtt), which cdk-nag cannot evaluate at synth time.',
            },
        ]);
    ...
    for (const path of [
        ...
        'HubInboundEndpoint/SecurityGroup/Resource',
