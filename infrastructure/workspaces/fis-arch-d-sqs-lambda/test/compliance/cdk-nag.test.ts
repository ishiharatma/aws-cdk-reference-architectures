...
function suppressBaseStack(stack: BaseStack): void {
    ...
    NagSuppressions.addResourceSuppressionsByPath(stack, `${p}/ProcessedRecordsTable/Resource`, [
        {
    ...
    // The dead-letter queue is the terminal point of the redrive chain by definition —
    // it cannot itself have a DLQ. This is exactly the scenario AwsSolutions-SQS3 exists
    // to distinguish; DLQs are the accepted exception to "every queue needs a DLQ".
    NagSuppressions.addResourceSuppressionsByPath(stack, `${p}/DeadLetterQueue/Resource`, [
        {
            id: 'AwsSolutions-SQS3',
            reason:
                'This queue IS the dead-letter queue for the main queue (see MainQueue RedrivePolicy). ' +
                'A DLQ has no further DLQ to redrive to by design.',
        },
    ]);
}
...
function suppressAppStack(stack: AppStack): void {
    ...
    // table.grantWriteData() and the SQS event source mapping generate wildcard permissions
    // on index/queue sub-resources (table/*, and the EventSourceMapping's own service role
    // policy for the queue). These are the standard grants CDK produces for these constructs.
    NagSuppressions.addStackSuppressions(
        stack,
        [
            {
                ...
                reason:
                    'Wildcard permissions are generated by table.grantWriteData() for DynamoDB index ' +
                    'sub-resources (table/*), and by the SQS event source mapping\'s standard grant. ' +
                    'The table and queue ARNs themselves are scoped to this workspace\'s resources.',
            },
        ],
        true,
    );

    // The producer's Function URL uses AWS_IAM auth (not NONE / public), which is the
    // AwsSolutions-recommended configuration — no suppression is needed for the auth type
    // itself. CORS allowedOrigins '*' is scoped to a demo load-generation endpoint that
    // requires signed IAM requests regardless of origin, so a wildcard origin does not
    // widen access.
}
