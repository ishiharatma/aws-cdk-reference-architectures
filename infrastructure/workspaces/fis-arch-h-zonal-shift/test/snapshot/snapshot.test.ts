...
/**
 * Snapshot tests for FIS chaos scenario H stacks.
 * Run `npm run test:snapshot:update` when intentional changes are made.
 */
describe('FIS Chaos Scenario H Stack Snapshots', () => {
    ...
    const appStack = new AppStack(app, 'TestAppStack', {
        ...
        impairedZoneHealthCheckBehavior: envParams.impairedZoneHealthCheckBehavior,
    });
    ...
    describe('AppStack', () => {
        ...
        test('Auto Scaling Group has AvailabilityZoneImpairmentPolicy with ZonalShiftEnabled', () => {
        ...
        test('Auto Scaling Group has a 100/150 instance maintenance policy', () => {
            template.hasResourceProperties('AWS::AutoScaling::AutoScalingGroup', {
    ...
    describe('FisStack', () => {
        ...
        test('Exactly 1 FIS experiment template is created', () => {
            template.resourceCountIs('AWS::FIS::ExperimentTemplate', 1);
        });

        test('The FIS template has a stop condition', () => {
        ...
        test('H-1 uses aws:network:disrupt-connectivity targeting aws:ec2:subnet', () => {
            const templates = template.findResources('AWS::FIS::ExperimentTemplate');
            ...
            expect(disruptTemplates).toHaveLength(1);
