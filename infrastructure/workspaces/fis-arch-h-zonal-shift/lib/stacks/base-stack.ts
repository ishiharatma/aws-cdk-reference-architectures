 ...
 *
 * Identical to Architecture G's BaseStack — same VPC/Aurora topology, reused
 * here as the foundation for Architecture H (ARC zonal shift on the ASG).
 * Aurora is provisioned with 1 writer + 1 reader, spread across the two
 * isolated subnets (one per AZ); H itself does not target Aurora with FIS
 * (see Architecture G's G-3 for that scenario), but the writer/reader
 * topology is kept for parity with G's base and for the demo DB
 * health-check endpoint on the EC2 instances.
 *
 * The VPC's private-with-egress subnets (one per AZ) are exposed as ARNs
 * because aws:network:disrupt-connectivity (H-1) targets resourceType
 * `aws:ec2:subnet` directly — there is no tag-based selection for this
 * action, so the FIS template needs the concrete subnet ARN for "AZ-1"
 * (the first AZ returned by the VPC).
 */
export class BaseStack extends cdk.Stack {
    ...
    /**
     * ARNs of `appSubnets`, in the same AZ order. FIS H-1 targets
    ...
    constructor(scope: Construct, id: string, props: BaseStackProps) {
        ...
        // AZs automatically. Kept for parity with Architecture G's base
        // (whose G-3 scenario targets this writer/reader pair); H itself
        // does not run an Aurora FIS scenario.
        ...
        new cdk.CfnOutput(this, 'AuroraClusterArn', {
            ...
            description: 'Aurora cluster ARN (not targeted by FIS in this workspace; see Architecture G G-3)',
        });
        ...
        new cdk.CfnOutput(this, 'Az1SubnetArn', {
            ...
            description: 'AZ-1 private-with-egress subnet ARN (FIS H-1 target)',
        });
