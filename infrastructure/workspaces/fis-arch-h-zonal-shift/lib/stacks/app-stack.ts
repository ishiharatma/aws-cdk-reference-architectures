...
export interface AppStackProps extends cdk.StackProps {
    ...
    /** ASG `AvailabilityZoneImpairmentPolicy.ImpairedZoneHealthCheckBehavior`. Defaults to `ReplaceUnhealthy`. */
 ...
 * Scaling Group (nginx, 2 AZs, ARC-zonal-shift-enabled) → Aurora PostgreSQL
 * Serverless v2.
 *
 ...
 *
 * Instances are tagged fis-target:app-instance for consistency with
 * Architecture G, though this workspace's own FIS template (H-1) targets
 * the subnet directly (`aws:network:disrupt-connectivity`), not the tag.
 *
 * The ASG carries `AvailabilityZoneImpairmentPolicy` (`ZonalShiftEnabled:
 * true`), which registers it with ARC zonal shift — see FisStack and the
 * README for how an operator-triggered zonal shift changes Auto Scaling's
 * replacement behavior during the same network-partition fault Architecture
 * G's G-2 scenario uses.
 */
export class AppStack extends cdk.Stack {
    ...
    constructor(scope: Construct, id: string, props: AppStackProps) {
        ...
        // the H-1 network-disruption experiment (same fault as Architecture G's G-2).
        ...
        userData.addCommands(
            ...
            '<!DOCTYPE html><html><head><title>FIS Chaos H</title></head><body>',
            '<h1>FIS Chaos Demo &#8212; Architecture H</h1>',
        ...
        this.asg = new autoscaling.AutoScalingGroup(this, 'AppAsg', {
            ...
            // AWS's documented best practice for zonal-shift-enabled ASGs: wait for a
            // replacement instance to be healthy before terminating the old one, so a
            // rolling replacement never drops below 100% of desired capacity.
        ...
        // Tag instances (kept for consistency with Architecture G's naming; H-1 targets the
        // subnet directly, not this tag).
        cdk.Tags.of(this.asg).add('fis-target', 'app-instance');

        // --- ARC zonal shift integration ---
        // `AvailabilityZoneImpairmentPolicy` is not yet exposed on the L2 AutoScalingGroup
        // construct (aws-cdk-lib 2.270.0) — set it via the underlying L1 CfnAutoScalingGroup.
        // ZonalShiftEnabled:true registers this ASG with ARC zonal shift (visible via
        // `aws arc-zonal-shift list-managed-resources`); ImpairedZoneHealthCheckBehavior
        // controls what happens to already-running instances in an AZ with an *active*
        // zonal shift — see the README for the full behavior table.
        ...
        new cdk.CfnOutput(this, 'AsgArn', {
            ...
            description: 'Auto Scaling Group ARN — resource-identifier for `aws arc-zonal-shift start-zonal-shift`',
        });
